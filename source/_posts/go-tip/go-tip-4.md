...
这一整块的代码风格非常类似**Try Exception**，即先写业务逻辑，在最后对错误进行集中处理。标准库中的`bufio.Scanner`就是参考这种方式实现的。

不过，由于Go语言对error的处理没有往外抛的机制，所以需要专门针对error做处理：

> 千万不要把panic的机制和错误处理混为一谈。
...
如果分别用一个词来形容前两种风格，我会用：

1. **过程式的调用**
2. **集中处理错误**

两种风格无法说清孰优孰劣，但会有更适宜的场景，我们来列举两种：
...
其中，第三种方式是一个很有意思的模式，可以帮助大家理解函数式编程的价值。
